...
def example_single_group_streaming():
    ...
    try:
        ...
        dataset = load_dataset(
            "BeeGass/Group-Theory-Collection", name="s5", streaming=True
        )
        ...
        for i, example in enumerate(train_stream):
            ...
            if isinstance(example, dict) and "input_sequence" in example:
                ...
                print(
                    f"  Group: {example['group_type']} (order {example['group_order']})"
                )
            else:
...
def example_filter_by_length_streaming():
    ...
    dataset = load_dataset(
        "BeeGass/Group-Theory-Collection", name="c10", streaming=True
    )

    # Filter for sequences of length 16
    filtered_stream = dataset["train"].filter(lambda x: x["sequence_length"] == 16)
...
def example_multiple_groups_streaming():
    ...
    # Load each group
    for group in groups:
        ds = load_dataset("BeeGass/Group-Theory-Collection", name=group, streaming=True)
...
def example_complexity_class_streaming():
    ...
    tc0_dataset = load_dataset(
        "BeeGass/Group-Theory-Collection", name="f20", streaming=True
    )
    ...
    nc1_dataset = load_dataset(
        "BeeGass/Group-Theory-Collection", name="m11", streaming=True
    )
...
def example_batch_processing_streaming():
    ...
    dataset = load_dataset("BeeGass/Group-Theory-Collection", name="a5", streaming=True)
...
def example_large_group_streaming():
    ...
    dataset = load_dataset("BeeGass/Group-Theory-Collection", name="s7", streaming=True)
