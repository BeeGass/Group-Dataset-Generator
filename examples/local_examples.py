...
def example_single_group_local():
    ...
    for i in range(min(5, len(train_data))):
        ...
        print(f"\nExample {i + 1}:")
        print(
            f"  Input: {example['input_sequence'][:50]}..."
            if len(example["input_sequence"]) > 50
            else f"  Input: {example['input_sequence']}"
        )
...
def example_filter_by_length_local():
    ...
    filtered_data = dataset["train"].filter(lambda x: x["sequence_length"] == 16)
...
def example_multiple_groups_local():
    ...
    for group, dataset in datasets.items():
        if "train" in dataset:
            ...
            print(f"\n{group.upper()}:")
            print(
                f"  Input: {example['input_sequence'][:30]}..."
                if len(example["input_sequence"]) > 30
                else f"  Input: {example['input_sequence']}"
            )
            print(f"  Target: {example['target']}")
    ...
    for dataset_path in sorted(dataset_dir.glob("*_data")):
        try:
            ...
            if "train" in dataset and len(dataset["train"]) > 0:
                ...
                print(
                    f"{group_name:<15} {example['group_order']:<10} {example['group_degree']:<10} {len(dataset['train']):<10} {len(dataset['test']):<10}"
                )
