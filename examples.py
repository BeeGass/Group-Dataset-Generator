...
def verify_dataset_composition(repo_id: str, config_name: str, num_samples: int = 5):
    ...
    dataset = load_dataset(
        repo_id, name=config_name, trust_remote_code=True, split="train"
    )
    ...
    for i in range(min(num_samples, len(dataset))):
        ...
        try:
            ...
            if i < 3:  # Show details for first 3 samples
                print(f"\nSample {i}:")
                print(
                    f"  Input IDs: {input_ids[:5]}{'...' if len(input_ids) > 5 else ''}"
                )
...
def run_examples():
    ...
    print(
        'ds = load_dataset("BeeGass/permutation-groups", name="s5_data", trust_remote_code=True)'
    )
    print("\n# Load all datasets:")
    print(
        'all_ds = load_dataset("BeeGass/permutation-groups", name="all", trust_remote_code=True)'
    )
    print("\n# Access splits:")
...
def run_verification():
    ...
    print(
        ...
        + (
            ...
            if all_passed
            else "❌ SOME DATASETS HAVE ERRORS!"
        )
    )
...
def benchmark_loading():
    ...
    print(
        f"Slowest: {max(times, key=times.get)} ({times[max(times, key=times.get)]:.2f}s)"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Examples for permutation-groups dataset"
    )
    parser.add_argument(
        "--verify", action="store_true", help="Run verification on all datasets"
    )
    parser.add_argument(
        "--benchmark", action="store_true", help="Benchmark loading times"
    )
